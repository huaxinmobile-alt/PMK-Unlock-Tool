    ...
    public partial class Form1 : Form
    {
        ...
        private void MobileBrandCombo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            var dynamicModels = (currentCategory == "Qualcomm")
                ? _loaderService.GetQualcommFolderModels(_loaderService.ResolveQualcommLoaderFolder(selectedBrand))
                : new List<string>();
            ...
            else if (_loaderService.ChipsetDatabase.ContainsKey(currentCategory) &&
                _loaderService.ChipsetDatabase[currentCategory].ContainsKey(selectedBrand))
            {
                foreach (var model in _loaderService.ChipsetDatabase[currentCategory][selectedBrand])
                {
                    mobileModelCombo.Items.Add(model);
                }
            }
        ...
        private void MobileModelCombo_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            string loaderPath = _loaderService.FindQualcommLoader(selectedBrand, selectedModel);
            ...
            if (profilePanel != null)
            {
                ...
                if (isChipsetCategory && mobileBrandCombo != null)
                {
                    ...
                    if (category == "Qualcomm")
                    {
                        ...
                        foreach (var folder in _loaderService.EnumerateLoaderBrands())
                        {
                            mobileBrandCombo.Items.Add(folder);
                        }
                    }
                    else if (_loaderService.ChipsetDatabase.ContainsKey(category))
                    {
                        foreach (var brand in _loaderService.ChipsetDatabase[category].Keys)
                        {
