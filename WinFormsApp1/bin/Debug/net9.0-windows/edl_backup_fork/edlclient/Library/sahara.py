...
class sahara(metaclass=LogBase):
    # Helper Class for Sahara Handshake
    class SimpleHello:
        def __init__(self, mode=0, version=2):
            self.mode = mode
            self.version = version
            self.cmd_packet_length = 0x30
            self.version_supported = 1
    ...
    # ===== PROACTIVE SAHARA CONNECT & DIRECT STREAM HANDSHAKE =====
    def connect(self):
        try:
            self.hello_sent = False
            self.pending_data = None
            v = self.cdc.read(length=0x30, timeout=1)
            
            if len(v) >= 8:
                if v[0] == 0x01: # SAHARA_HELLO_REQ
                ...
                elif v[0] in [0x03, 0x05, 0x06, 0x09, 0x0A]:
                    self.pending_data = v
                    self.hello_sent = True
                    rsp = sahara.SimpleHello(mode=sahara_mode_t.SAHARA_MODE_IMAGE_TX_PENDING, version=2)
                    return {"mode": "sahara", "cmd": cmd_t.SAHARA_HELLO_REQ, "data": rsp}
                elif b"<?xml" in v:
                    return {"mode": "firehose"}
                elif v[0] == 0x04:  # Passive target HELLO_RESP (cmd 0x04 / protocol 13)
                    rsp = sahara.SimpleHello(mode=sahara_mode_t.SAHARA_MODE_IMAGE_TX_PENDING, version=13)
                    return {"mode": "sahara", "cmd": cmd_t.SAHARA_HELLO_REQ, "data": rsp}
                elif v[0] == 0x7E:
                    return {"mode": "nandprg"}

            # Send Hello Response Kickstart
            self.cmd_hello(sahara_mode_t.SAHARA_MODE_IMAGE_TX_PENDING, version=2)
            self.hello_sent = True

            # Wait for the target reply (retry: some targets answer slowly)
            res = b""
            for _ in range(8):
                res = self.cdc.read(length=0x30, timeout=1)
                if len(res) >= 8:
                    break
            if len(res) >= 8:
                if res[0] == 0x01:
                    pkt = self.ch.pkt_cmd_hdr(res)
                    rsp = self.ch.pkt_hello_req(res)
                    self.pktsize = rsp.cmd_packet_length
                    self.version = rsp.version
                    self.info(f"Protocol version: {rsp.version}, Version supported: {rsp.version_supported}")
                    return {"mode": "sahara", "cmd": cmd_t.SAHARA_HELLO_REQ, "data": rsp}
                elif res[0] in [0x03, 0x05, 0x06, 0x09, 0x0A]:
                    self.pending_data = res
                    rsp = sahara.SimpleHello(mode=sahara_mode_t.SAHARA_MODE_IMAGE_TX_PENDING, version=2)
                    return {"mode": "sahara", "cmd": cmd_t.SAHARA_HELLO_REQ, "data": rsp}
                elif b"<?xml" in res:
                    return {"mode": "firehose"}
                elif res[0] == 0x04:  # Passive target HELLO_RESP (cmd 0x04 / protocol 13)
                    rsp = sahara.SimpleHello(mode=sahara_mode_t.SAHARA_MODE_IMAGE_TX_PENDING, version=13)
                    return {"mode": "sahara", "cmd": cmd_t.SAHARA_HELLO_REQ, "data": rsp}
                elif res[0] == 0x7E:
                    return {"mode": "nandprg"}
                else:
                    self.debug(f"Unhandled reply byte: 0x{res[0]:02x} ({res.hex()})")
            else:
                self.debug("No valid reply after kickstart")
    ...
    def upload_loader(self, version):
        ...
        if not hasattr(self, 'hello_sent') or not self.hello_sent:
        ...
        try:
            loop = 0
            empty_retry = 0
            max_empty_retries = 30  # Timeout threshold (approx 3-5 seconds)
                ...
                if not resp:
                    if loop > 0:
                        ...
                        if empty_retry >= max_empty_retries:
                            self.error("Device stopped responding after sending loader header.")
                            self.error("Possible reason: Loader signature was REJECTED by device (Incompatible loader or Auth required).")
                            return "error"
                    continue
