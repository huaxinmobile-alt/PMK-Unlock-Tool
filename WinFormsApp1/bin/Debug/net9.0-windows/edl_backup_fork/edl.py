...
# UTF-8 Console output encoding fix for Windows
...
class main(metaclass=LogBase):
    ...
    # ===== ACCURATE QUALCOMM CHIPSET & HWID AUTO-LOADER MATCHER =====
    def find_auto_loader(self, chip_keywords=None, msm_id=None):
        possible_dirs = [
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "Loaders"),
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "Loaders"),
            os.path.join(os.getcwd(), "Loaders"),
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "edlclient", "Loaders")
        ]
        ...
        all_loaders = []
        for root, dirs, files in os.walk(loaders_dir):
            for file in files:
                if file.lower().endswith(('.elf', '.mbn', '.bin')):
                    all_loaders.append(os.path.join(root, file))
                    
        if not all_loaders:
            return None
            
        # Priority: "No Auth" -> "Auth Skip" -> "SIG" -> Other
        all_loaders.sort(key=lambda x: (
            0 if "no auth" in x.lower() else
            1 if "auth skip" in x.lower() else
            2 if "sig" in x.lower() else 3
        ))
        
        # Dedicated Qualcomm Chipset Mapping Table
        chip_to_models = {
            0x000bf0e1: ["839", "439", "REDMI 8A", "REDMI 8", "REDMI 7A", "SDM439"],
            0x0009b0e1: ["839", "8937", "439", "REDMI 8A", "REDMI 8", "REDMI 7A", "SDM439"],
            0x000460e1: ["8937", "8940", "REDMI 3S", "REDMI 4", "REDMI 4X", "REDMI 4A"],
            0x0004f0e1: ["8953", "REDMI 4 PRIME", "REDMI NOTE 4", "REDMI 5 PLUS", "MI A1", "MI MAX 2", "MI 5X"],
            0x000940e1: ["660", "REDMI NOTE 7", "MI 6X", "MI 8 LITE", "MI A2", "MI MAX 3", "MI PAD 4"],
            0x0010a0e1: ["6125", "665", "REDMI NOTE 8", "REDMI 9T", "REDMI 9 POWER", "MI A3", "MI CC9E"],
            0x0011e0e1: ["720", "720G", "REDMI NOTE 9S", "REDMI NOTE 9 PRO", "POCO M2 PRO"],
            0x0014b0e1: ["7325", "778G", "MI 11 LITE 5G NE", "POCO X5 PRO"],
            0x001260e1: ["6150", "730", "730G", "REDMI K20", "MI 9T", "POCO X2", "REDMI K30"],
            0x0008e0e1: ["845", "POCO F1", "MI 8", "MI MIX 2S", "MI MIX 3"],
            0x000be0e1: ["855", "MI 9", "REDMI K20 PRO", "MI 9T PRO"],
            0x0011b0e1: ["865", "MI 10", "MI 10 PRO", "REDMI K30 PRO", "POCO F2 PRO"],
            0x0013b0e1: ["888", "MI 11", "REDMI K40 PRO", "MI 11 ULTRA"]
        }

        search_kws = []
        if msm_id and msm_id in chip_to_models:
            search_kws.extend(chip_to_models[msm_id])
        if chip_keywords:
            search_kws.extend(chip_keywords)

        if search_kws:
            for kw in search_kws:
                if not kw or len(kw) < 2: continue
                for l in all_loaders:
                    fname = os.path.basename(l).lower()
    ...
    def run(self):
        ...
        if self.args["--loader"] in ['None', None, '']:
            self.info("[*] [Auto-Engine] Auto detection of loader enabled...")
            ...
            if cmd == cmd_t.SAHARA_HELLO_REQ:
                if "data" in conninfo:
                    ...
                    if data.mode == sahara_mode_t.SAHARA_MODE_MEMORY_DEBUG:
                        ...
                        if self.sahara.programmer != "" and (getattr(self.sahara, 'hello_sent', False) or getattr(data, 'mode', 0) == sahara_mode_t.SAHARA_MODE_IMAGE_TX_PENDING):
                            mode = self.sahara.upload_loader(version=version)
                        else:
                            sahara_info = self.sahara.cmd_info(version=version)
                            if sahara_info is not None:
                                # Auto match exact loader from Loaders folder by HWID / MSM_ID
                                if self.sahara.programmer == "":
                                    keywords = []
                                    if getattr(self.sahara, 'msm_str', None): keywords.append(self.sahara.msm_str)
                                    if getattr(self.sahara, 'hwidstr', None): keywords.append(self.sahara.hwidstr[:8])
                                    if getattr(self.sahara, 'msm_id', None) and self.sahara.msm_id in msmids:
                                        cpu_name = msmids[self.sahara.msm_id]
                                        keywords.extend(cpu_name.replace("/", " ").replace("-", " ").split())
                                    
                                    auto_loader = self.find_auto_loader(keywords, getattr(self.sahara, 'msm_id', None))
                                    if auto_loader:
                                        self.sahara.programmer = auto_loader
                                        self.info(f"[+] [Auto Loader Matched]: {os.path.basename(auto_loader)}")
                                ...
                                if self.sahara.programmer == "":
                                    auto_loader = self.find_auto_loader(msm_id=getattr(self.sahara, 'msm_id', None))
                                    if auto_loader:
                                        self.sahara.programmer = auto_loader
                                        self.info(f"[+] [Auto Loader Matched]: {os.path.basename(auto_loader)}")
                                        mode = self.sahara.upload_loader(version=version)
